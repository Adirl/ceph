...
RDMADispatcher::~RDMADispatcher()
{
  ...
  stack->get_infiniband()->set_dispatcher(nullptr);
}
...
{
  tx_cc = stack->get_infiniband()->create_comp_channel(cct);
  assert(tx_cc);
  rx_cc = stack->get_infiniband()->create_comp_channel(cct);
  assert(rx_cc);
  tx_cq = stack->get_infiniband()->create_comp_queue(cct, tx_cc);
  assert(tx_cq);
  rx_cq = stack->get_infiniband()->create_comp_queue(cct, rx_cc);
  assert(rx_cq);
...
void RDMADispatcher::handle_async_event()
{
  ...
  while (1) {
    ...
    if (ibv_get_async_event(stack->get_infiniband()->get_device()->ctxt, &async_event)) {
      if (errno != EAGAIN)
    ...
    if (async_event.event_type == IBV_EVENT_QP_LAST_WQE_REACHED) {
      ...
      ldout(cct, 1) << __func__ << " ibv_get_async_event: dev=" << stack->get_infiniband()->get_device()->ctxt
...
void RDMADispatcher::post_chunk_to_pool(Chunk* chunk) {
  Mutex::Locker l(lock);
  stack->get_infiniband()->post_chunk_to_pool(chunk);
...
void RDMADispatcher::polling()
{
  ...
  while (true) {
    ...
    if (rx_ret > 0) {
      ...
      stack->get_infiniband()->post_chunks_to_srq(rx_ret);
      for (int i = 0; i < rx_ret; ++i) {
	    ...
	    polled[conn].push_back(*response);
          ...
          ldout(cct, 1) << __func__ << " work request returned error for buffer(" << chunk
              ...
              << stack->get_infiniband()->wc_status_to_string(response->status) << ")" << dendl;
          conn = get_conn_lockless(response->qp_num);
...
//In the case of 'fin' wr_id points to the QueuePair.
    if (stack->get_infiniband()->get_memory_manager()->is_tx_buffer(chunk->buffer)) {
...
void RDMADispatcher::post_tx_buffer(std::vector<Chunk*> &chunks)
{
  ...
  stack->get_infiniband()->get_memory_manager()->return_tx(chunks);
...
{
  stack->get_infiniband()->init();

  auto p = new RDMAServerSocketImpl(cct, stack->get_infiniband(), get_stack()->get_dispatcher(), this, sa);
...
int RDMAWorker::connect(const entity_addr_t &addr, const SocketOptions &opts, ConnectedSocket *socket)
{
  ...
  RDMAConnectedSocketImpl* p = new RDMAConnectedSocketImpl(cct, stack->get_infiniband(), get_stack()->get_dispatcher(), this);
