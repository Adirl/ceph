...
RDMADispatcher::RDMADispatcher(CephContext* c, Infiniband* infiniband)
  : cct(c), ib(infiniband),async_handler(new C_handle_cq_async(this)), lock("RDMADispatcher::lock"),
...
RDMAStack::RDMAStack(CephContext *cct, const string &t): NetworkStack(cct, t)
{
  ...
  ldout(cct, 20) << __func__ << " constructing IB context" << dendl;
  dispatcher = ib->get_dispatcher();
