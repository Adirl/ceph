...
RDMAConnectedSocketImpl::~RDMAConnectedSocketImpl()
{
  ...
  for (unsigned i=0; i < wc.size(); ++i) {
    ret = infiniband->post_chunk_to_srq(reinterpret_cast<Chunk*>(wc[i].wr_id));
    assert(ret == 0);
    dispatcher->rx_buf_release(1);
  }
  for (unsigned i=0; i < buffers.size(); ++i) {
    ret = infiniband->post_chunk_to_srq(buffers[i]);
    assert(ret == 0);
    dispatcher->rx_buf_release(1);
  }
}
...
ssize_t RDMAConnectedSocketImpl::read(char* buf, size_t len)
{
  ...
  for (size_t i = 0; i < cqe.size(); ++i) {
    ...
    if (response->byte_len == 0) {
      ...
      assert(infiniband->post_chunk_to_srq(chunk) == 0);
      dispatcher->rx_buf_release(1);
    } else {
      if (read == (ssize_t)len) {
...
ssize_t RDMAConnectedSocketImpl::read_buffers(char* buf, size_t len)
{
  ...
  for (; c != buffers.end() ; ++c) {
    ...
    if ((*c)->over()) {
      assert(infiniband->post_chunk_to_srq(*c) == 0);
      dispatcher->rx_buf_release(1);
